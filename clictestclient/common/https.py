...
class HTTPSAdapter(adapters.HTTPAdapter):
    ...
    clictestclient's one.
    """
    def __init__(self, *args, **kwargs):
        classes_by_scheme = poolmanager.pool_classes_by_scheme
        classes_by_scheme["clictest+https"] = HTTPSConnectionPool
    ...
    def _create_clictest_httpsconnectionpool(self, url):
    ...
    def get_connection(self, url, proxies=None):
        try:
            ...
            # The scheme is most likely clictest+https but check anyway
            if not url.startswith('clictest+https://'):
                raise

            return self._create_clictest_httpsconnectionpool(url)
...
class HTTPSConnectionPool(connectionpool.HTTPSConnectionPool):
    ...
    returns an instances of clictestclient's VerifiedHTTPSConnection
