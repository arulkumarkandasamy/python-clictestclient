...
"""
Command-line interface to the OpenStack Tests API.
"""
...
import clictestclient
from clictestclient._i18n import _
...
class OpenStackTestsShell(object):
    ...
    def get_base_parser(self):
        parser = argparse.ArgumentParser(
            ...
            epilog='See "clictest help COMMAND" '
        ...
        parser.add_argument('--version',
                            action='version',
                            version=clictestclient.__version__)

        parser.add_argument('-d', '--debug',
                            default=bool(utils.env('CLICTESTCLIENT_DEBUG')),
                            action='store_true',
                            help='Defaults to env[CLICTESTCLIENT_DEBUG].')
        ...
        parser.add_argument('--os-test-url',
                            default=utils.env('OS_TEST_URL'),
                            help=('Defaults to env[OS_TEST_URL]. '
                                  'If the provided test url contains '
                                  'a version number and '
                                  '`--os-test-api-version` is omitted '
                                  'the version of the URL will be picked as '
                                  'the test api version to use.'))

        parser.add_argument('--os_test_url',
                            help=argparse.SUPPRESS)

        parser.add_argument('--os-test-api-version',
                            default=utils.env('OS_TEST_API_VERSION',
                                              default=None),
                            help='Defaults to env[OS_TEST_API_VERSION] or 2.')

        parser.add_argument('--os_test_api_version',
                            help=argparse.SUPPRESS)
        ...
        if osprofiler_profiler:
            parser.add_argument('--profile',
                                ...
                                'clictest, it is specified in paste '
                                'configuration file at '
                                '/etc/clictest/api-paste.ini'
                                'Without key '
    ...
    def _get_test_url(self, args):
        """Translate the available url-related options into a single string.

        Return the endpoint that should be used to talk to Clictest if a
        clear decision can be made. Otherwise, return None.
        """
        if args.os_test_url:
            return args.os_test_url
        else:
            return None
    ...
    def _get_versioned_client(self, api_version, args):
        endpoint = self._get_test_url(args)
        ...
        return clictestclient.Client(api_version, endpoint, **kwargs)
    ...
    def main(self, argv):
        ...
        try:
            ...
            # test-url first. If no version was specified, fallback
            # to the api-test-version arg. If both of these fail then
            # fallback to the minimum supported one and let keystone
            # do the magic.
            endpoint = self._get_test_url(options)
        ...
        try:
            api_version = int(options.os_test_api_version or url_version or 2)
        ...
        if not options.os_test_api_version and api_version == 2:
            switch_version = True
        ...
        LOG = logging.getLogger('clictestclient')
    ...
    def do_help(self, args, parser):
        ...
        if not args.os_test_api_version or args.os_test_api_version == '2':
            # NOTE(NiallBunting) This currently assumes that the only versions
            # are one and two.
            try:
                if command is None:
                    print("\nRun `clictest --os-test-api-version 1 help`"
                          " for v1 help")
                else:
                    self.get_subcommand_parser(1)
                    if command in self.subcommands:
                        command = ' ' + command
                        print(("\nRun `clictest --os-test-api-version 1 help%s`"
    ...
    def do_bash_completion(self, _args):
        ...
        clictest.bash_completion script doesn't have to hard code them.
        """
        commands = set()
        options = set()
...
def main():
    try:
        ...
        OpenStackTestsShell().main(argv)
    except KeyboardInterrupt:
        utils.exit('... terminating clictest client', exit_code=130)
